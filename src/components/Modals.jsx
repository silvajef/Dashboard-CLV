...
import { TIPOS_VEICULO, TIPOS_MANUT, COMBUSTIVEIS, STATUS_VEICULO_CFG, STATUS_SERV_CFG, C, today, fmtR } from '../lib/constants'
import { custoV } from '../lib/constants'
...
export function ModalVeiculo({ data, onSave, onClose, loading }) {
  const [f, setF] = useState(data || {
    ...
    valor_estoque: '', data_entrada: today(), obs: '', status: 'pendente'
  })
  const set = (k, v) => setF(p => ({ ...p, [k]: v }))

  const handleSave = () => {
    if (!f.placa || !f.modelo) return
    onSave({
      ...f,
      km: Number(f.km) || 0,
      ano: Number(f.ano) || 0,
  ...
  return (
    <Modal title={data ? 'Editar Veículo' : 'Novo Veículo'} wide onClose={onClose}>
      ...
      <Input label="Observações" value={f.obs} onChange={v => set('obs', v)} placeholder="Informações adicionais..." style={{ marginTop: 14 }} rows={2} />
  ...
  return (
    <Modal title={`Registrar Venda — ${data.modelo}`} onClose={onClose}>
      ...
      <div style={{ marginTop: 16, background: C.surface, borderRadius: 8, padding: 14, fontSize: 13 }}>
        {[['Custo de Manutenção', fmtR(custo), C.muted], ['Valor de Estoque', fmtR(data.valor_estoque), C.muted], ['Lucro Estimado', fmtR(lucro), lucro >= 0 ? C.green : C.red]].map(([l, v, c]) => (
...
export function ModalServico({ data, veiculoId, prestadores, onSave, onClose, loading }) {
  const [f, setF] = useState(data || {
    veiculo_id: veiculoId, tipo: '', descricao: '', data_servico: today(),
    prestador_id: '', custo_pecas: 0, custo_mao: 0, outros: 0,
  ...
  return (
    <Modal title={data ? 'Editar Serviço' : 'Registrar Serviço'} wide onClose={onClose}>
      ...
      <Input label="Prestador / Oficina" value={f.prestador_id} onChange={v => set('prestador_id', Number(v) || null)}
      ...
      <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 14, marginTop: 14 }}>
        ...
        <div style={{ background: '#f59e0b18', border: '1px solid #f59e0b33', borderRadius: 8, padding: '10px 14px', display: 'flex', flexDirection: 'column', justifyContent: 'center' }}>
          ...
          <div style={{ fontSize: 22, fontWeight: 800, color: '#f59e0b', fontFamily: "'JetBrains Mono',monospace" }}>{fmtR(total)}</div>
        </div>
      </div>
    ...
    <Modal title={data ? 'Editar Prestador' : 'Novo Prestador'} onClose={onClose}>
      ...
      <div style={{ display: 'flex', gap: 10, marginTop: 24 }}>
        ...
        <Btn style={{ flex: 1 }} onClick={() => onSave({ ...f, avaliacao: Number(f.avaliacao) })} disabled={loading}>
          {loading ? 'Salvando...' : data ? 'Salvar Alterações' : 'Cadastrar Prestador'}
        </Btn>
      </div>
    </Modal>
  )
...
/* ── Modal Confirmar ────────────────────────────────────────────── */
export function ModalConfirm({ title, message, onConfirm, onClose, loading, variant = 'danger' }) {
  return (
    <Modal title={title} onClose={onClose}>
      <p style={{ color: C.muted, fontSize: 14 }}>{message}</p>
      <div style={{ display: 'flex', gap: 10, marginTop: 20 }}>
        <Btn variant="secondary" style={{ flex: 1 }} onClick={onClose}>Cancelar</Btn>
        <Btn variant={variant} style={{ flex: 1 }} onClick={onConfirm} disabled={loading}>
