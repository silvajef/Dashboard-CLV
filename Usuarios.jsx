...
const ROLE_INFO = {
  ...
  operador:     { label: 'Operador',     cor: C.accent  },
  visualizador: { label: 'Visualizador', cor: C.muted   },
}

// Descrição de permissões por role — exibida no card ao criar/editar
const PERMISSOES = {
  admin: [
...
export default function Usuarios() {
  ...
  const [modal,     setModal]     = useState(null) // null | 'novo' | { editar: usuario }
  ...
  async function criarUsuario({ email, senha, nome, role }) {
    ...
    // Garante que o profile existe com os dados corretos
    // (o trigger handle_new_user pode ter uma latência de milissegundos)
    if (data.user) {
  ...
  return (
    <div style={s.page}>
      ...
      <div style={s.header}>
        ...
        <button onClick={() => setModal('novo')} style={s.btnNovo}>
          + Novo usuário
        </button>
      </div>
      ...
      {/* Lista */}
      {loading ? (
        <p style={{ color: C.muted, padding: 24, fontFamily: "'Syne', sans-serif" }}>
          Carregando...
        </p>
      ) : (
        <div style={s.tabela}>
              ...
              <div key={u.id} style={{ ...s.tRow, opacity: u.ativo ? 1 : 0.45 }}>
                ...
                <div style={{ flex: 2 }}>
                  <p style={s.nomeUsuario}>
                    {u.nome || '(sem nome)'}
                    {ehVoce && (
                      <span style={s.badgeVoce}>você</span>
                    )}
                  </p>
                ...
                <div style={{ flex: 1 }}>
                  <span style={{
                    ...s.badge,
                    background: u.ativo ? `${C.success}15` : `${C.muted}15`,
                ...
                <div style={{ flex: 1, display: 'flex', justifyContent: 'flex-end', gap: 6 }}>
                  ...
                  {!ehVoce && (
                    <button
                      ...
                      style={{ ...s.btnAcao, color: u.ativo ? C.warn : C.success }}
      ...
      <p style={s.nota}>
        💡 Para ver os e-mails dos usuários acesse Supabase → Authentication → Users.
      </p>
      ...
      {modal === 'novo' && (
        <ModalNovoUsuario
          ...
          onSalvar={async dados => {
            ...
            if (res.ok) {
              setModal(null)
              exibirToast('✅ Usuário criado! Verifique o e-mail de confirmação.')
            }
            return res
          }}
        />
...
// ─── Modal: Novo Usuário ──────────────────────────────────────────────────────
function ModalNovoUsuario({ onClose, onSalvar }) {
  ...
  return (
    <Overlay onClose={onClose}>
      ...
      <form onSubmit={handleSubmit} style={s.modalForm}>
        <MField label="Nome completo">
          <MInput value={form.nome} onChange={v => setForm(p => ({...p, nome: v}))}
            placeholder="Nome do funcionário" required />
        </MField>
        <MField label="E-mail">
          <MInput type="email" value={form.email} onChange={v => setForm(p => ({...p, email: v}))}
            placeholder="email@empresa.com" required />
        </MField>
        <MField label="Senha (mínimo 6 caracteres)">
          <MInput type="password" value={form.senha} onChange={v => setForm(p => ({...p, senha: v}))}
            placeholder="••••••••" required />
        </MField>
        <MField label="Permissão">
          <select value={form.role} onChange={e => setForm(p => ({...p, role: e.target.value}))}
            style={{ ...s.mInput, cursor: 'pointer' }}>
        ...
        <div style={s.modalBtns}>
          ...
          <button type="submit" disabled={saving} style={{ ...s.btnSalvar, flex: 1, opacity: saving ? 0.6 : 1 }}>
...
// ─── Modal: Editar Usuário ────────────────────────────────────────────────────
function ModalEditarUsuario({ usuario, ehVoce, onClose, onSalvar }) {
      ...
      <form onSubmit={handleSubmit} style={s.modalForm}>
        ...
        <MField label="Permissão">
          <select
            ...
            disabled={ehVoce}
            style={{ ...s.mInput, cursor: ehVoce ? 'not-allowed' : 'pointer', opacity: ehVoce ? 0.5 : 1 }}
          >
            <option value="admin">Admin — acesso total</option>
            <option value="operador">Operador — cria e edita, sem excluir</option>
            <option value="visualizador">Visualizador — somente leitura</option>
...
function PermCard({ role }) {
  ...
  return (
    <div style={{ background: C.bg, border: `1px solid ${C.border}`, borderRadius: 8, padding: '12px 14px' }}>
      <p style={{ fontSize: 11, color: C.muted, margin: '0 0 8px', fontWeight: 700,
        textTransform: 'uppercase', letterSpacing: '0.08em' }}>
        Permissões deste role
      </p>
      {lista.map((p, i) => (
        <div key={i} style={{ display: 'flex', alignItems: 'flex-start', gap: 8, marginBottom: 5 }}>
          <span style={{ color: p.ok ? C.success : C.danger, fontSize: 12, marginTop: 1, flexShrink: 0 }}>
            {p.ok ? '✓' : '✗'}
          </span>
          <span style={{ fontSize: 12, color: p.ok ? C.text : C.muted, lineHeight: 1.4 }}>
            {p.texto}
          </span>
        </div>
      ))}
  ...
  return (
    <div
      style={{ position: 'fixed', inset: 0, background: 'rgba(0,0,0,0.75)',
        ...
        zIndex: 1000, padding: 16, fontFamily: "'Syne', sans-serif" }}
...
function MInput({ onChange, ...props }) {
  ...
  return (
    <input
      {...props}
      style={{ ...s.mInput, borderColor: focused ? C.accent : C.border }}
...
const s = {
  ...
  btnNovo:    { background: C.accent, border: 'none', borderRadius: 8,
                padding: '9px 18px', color: '#fff', fontSize: 13, fontWeight: 700,
                fontFamily: "'Syne', sans-serif", cursor: 'pointer' },
  toast:      { background: `${C.success}18`, border: `1px solid ${C.success}35`,
                color: C.success, borderRadius: 8, padding: '10px 16px',
                fontSize: 13, fontWeight: 600, marginBottom: 16 },
  tabela:     { background: C.surface, border: `1px solid ${C.border}`, borderRadius: 10, overflow: 'hidden' },
  tHead:      { display: 'flex', gap: 12, padding: '10px 16px',
                borderBottom: `1px solid ${C.border}`, fontSize: 11, fontWeight: 700,
                color: C.muted, textTransform: 'uppercase', letterSpacing: '0.08em' },
  tRow:       { display: 'flex', gap: 12, padding: '12px 16px', alignItems: 'center',
                borderBottom: `1px solid ${C.border}`, transition: 'background 0.1s' },
  nomeUsuario:{ margin: 0, fontSize: 14, fontWeight: 600, color: C.text, display: 'flex',
                alignItems: 'center', gap: 6, flexWrap: 'wrap' },
  idTexto:    { margin: '2px 0 0', fontSize: 10, color: C.border,
                fontFamily: "'JetBrains Mono', monospace" },
  badgeVoce:  { fontSize: 10, color: C.accent, fontWeight: 700,
                background: `${C.accent}20`, padding: '1px 6px', borderRadius: 10 },
  badge:      { fontSize: 11, fontWeight: 700, padding: '3px 8px', borderRadius: 20 },
  btnAcao:    { background: 'none', border: `1px solid ${C.border}`, borderRadius: 6,
                padding: '4px 8px', fontSize: 14, cursor: 'pointer', color: C.muted,
  ...
  mInput:     { background: C.bg, border: `1px solid ${C.border}`, borderRadius: 8,
                padding: '10px 14px', color: C.text, fontSize: 14,
                fontFamily: "'Syne', sans-serif", outline: 'none',
                width: '100%', boxSizing: 'border-box' },
  erroTexto:  { margin: 0, color: C.danger, fontSize: 13, fontWeight: 500 },
  modalBtns:  { display: 'flex', gap: 10, marginTop: 4 },
  btnCancelar:{ background: 'none', border: `1px solid ${C.border}`, borderRadius: 8,
                padding: '10px 16px', color: C.muted, fontSize: 13, fontWeight: 600,
                fontFamily: "'Syne', sans-serif", cursor: 'pointer' },
  btnSalvar:  { background: C.accent, border: 'none', borderRadius: 8,
